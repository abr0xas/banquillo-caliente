...
export default function GTATimer({
    ...
    const TimeUnit = ({ value, label, showSeparator = true }: { value: number, label: string, showSeparator?: boolean }) => (
        <div class="flex items-center">
            <div class="flex flex-col items-center">
                <div class="text-4xl sm:text-7xl md:text-8xl font-sans font-black leading-none tabular-nums tracking-tighter"
                     style="color: white; text-shadow: 1.1px 0px 0px #cc2200;">
            ...
            {showSeparator && (
                <div class="text-2xl sm:text-6xl md:text-7xl font-sans font-black px-1 sm:px-4 md:px-6 self-start mt-1 mb-6"
                     style="color: white; text-shadow: 1.1px 0px 0px #cc2200;">
                    :
                </div>
            )}
    ...
    return (
        <div class="w-full codec-panel py-6 sm:py-10 px-4 relative overflow-hidden group">
            {/* Corner Markers */}
            <div class="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-inferno opacity-50" />
            <div class="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-inferno opacity-50" />
            <div class="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-inferno opacity-50" />
            <div class="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-inferno opacity-50" />

            <div class="max-w-4xl mx-auto flex flex-col items-center">
                <div class="flex items-center gap-3 mb-8">
                    <div class="w-1.5 h-1.5 bg-inferno-ember animate-pulse rounded-full" />
                ...
                <div class="mt-6 sm:mt-10 flex gap-0.5 sm:gap-1.5 items-center opacity-20 overflow-hidden w-full justify-center">
                    {[...Array(12)].map((_, i) => (
                        <div key={i} class={`h-0.5 sm:h-1 w-4 sm:w-8 bg-inferno shrink-0 ${i % 4 === 0 ? 'animate-pulse' : ''}`} />
                    ))}
                </div>
            </div>
        </div>
